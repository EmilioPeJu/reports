...
async def three_evenly_spaced_pulses(dut, repeats=1):
    ...
    await setup_table(dut, 0, 1*4, more=False)
    ...
    assert dut.active_o.value == 0
    await wait_for_state(dut, State.WAIT_ENABLE, timeout=2)
...
async def simple_streaming(dut):
    ...
    await setup_table(dut, 0, 2*4, more=True)
    await wait_for_state(dut, State.WAIT_ENABLE)
    ...
    dut.enable_i.value = 1
    await clkedge
    await setup_table(dut, 0x100, 2*4, more=False)
    await clkedge
...
async def simple_streaming_disabled(dut):
    ...
    await setup_table(dut, 0, len(entries), more=True)
    await wait_for_state(dut, State.WAIT_ENABLE)
