...
def get_block_ini(panda_src: str, module: str) -> ConfigParser:
    ...
    """
    ini_path = Path(panda_src) / 'modules' / module / "{}.block.ini".format(module)
...
def get_extra_signals_info(module: str, panda_src: str,
...
async def section_timing_test(
    ...
    if block_has_dma(block_ini):
        dma_driver = DMADriver(dut, panda_src_dir, module)

    signals_info = get_signals_info(block_ini)
    signals_info.update(get_extra_signals_info(module, panda_src_dir,
                                               panda_build_dir))
...
async def module_timing_test(dut: Dut):
    ...
    panda_src_dir = os.getenv("panda_src_dir")
    ...
    block_ini = get_block_ini(panda_src_dir, module)
    timing_inis = get_timing_inis(panda_src_dir, module)
    timing_ini = timing_inis[timing_ini_path]
    if test_name.strip() != ".":
        await section_timing_test(
            dut, module, test_name, block_ini, timing_ini, panda_src_dir, panda_build_dir, collect
        )
